#define DMX_FIXTURE_CNT 11
#define DMX_FIXT_LIST                           \
  {                                             \
    1, 33, 65, 97, 129, 193, 225, 161, 257, 300, 310 \
  }
#define DMX_FIXT_CHNL 5

#define VarMacro_BYTES (DMX_FIXTURE_CNT * DMX_FIXT_CHNL)

#include "C:\Users\AVG\Documents\Electrical Main (Amir)\Arduino\Projects\FSCC\328P_LightControl\V1\Core\VarMacro\VarMacro_1.0.1\VarMacro.h"
#include "C:\Users\AVG\Documents\Electrical Main (Amir)\Arduino\Projects\FSCC\328P_LightControl\V1\Core\DMX_Driver\DMX_Driver_1.0.1\DMX_Driver.h"
#include "C:\Users\AVG\Documents\Electrical Main (Amir)\Arduino\Projects\FSCC\328P_LightControl\V1\Core\UDP_Parse_1.0.0\UDP_Parse.h"
#include "C:\Users\AVG\Documents\Electrical Main (Amir)\Arduino\Projects\FSCC\328P_LightControl\V1\Core\BUTTON_INTERFACE_1.0.3\BUTTON_INTERFACE.h"
#include "C:\Users\AVG\Documents\Electrical Main (Amir)\Arduino\Projects\FSCC\328P_LightControl\V1\Core\LED_MACROS\LED_MACROS_4.1.1_1\LED_MACROS.h"
#include <EEPROM.h>
  ...
  while (UDP.UDP_Avail())
  {
    ...
    if (Bytes >= 4)
    {
      ...
      if (!strcmp(CmdStr, "FxHx"))
      {
        ...
        for (uint8_t X = 0; X < 5; X++)
        {
          Serial.print(Chnls[X]);
          Serial.print(' ');
        }
        Serial.println();
        DmxMacro.StopMacro();
      }
      if (!strcmp(CmdStr, "FtSt"))
      {
        Serial.println("FtSt");
        uint8_t ParseIndex = 5;
        uint8_t Fixt = ParseInt(Buff, ParseIndex, Bytes);
        ...
        Serial.print(' ');
        Serial.println(Val);
        DMX_TAR_BUFF[Dmx.ChnlAdd(Fixt, Chnl)] = Val;
        DmxMacro.StopMacro();
      }
      if (!strcmp(CmdStr, "FtHx"))
      {
        Serial.println("FtHx");
        uint8_t ParseIndex = 5;
        uint8_t Fixt = ParseInt(Buff, ParseIndex, Bytes);
        ...
        {
          Chnls[X] = ParseHex(Buff, ParseIndex, Bytes);
          DMX_TAR_BUFF[Dmx.ChnlAdd(Fixt, X)] = Chnls[X];
        }
        Serial.print(Fixt);
        Serial.print(' ');
        for (uint8_t X = 0; X < 5; X++)
      ...
      if (!strcmp(CmdStr, "FtCl"))
      {
        Serial.println("FtCl");
        for (uint8_t x = 0; x < (DMX_FIXTURE_CNT * DMX_FIXT_CHNL); x++)
        {
          DMX_TAR_BUFF[x] = DMX_BUFF[x];
        }
        DmxMacro.StopMacro();
      }
      if (!strcmp(CmdStr, "FtRn"))
      {
        Serial.println("FtRn");
        uint8_t ParseIndex = 5;
        uint32_t MacroDelay = ParseLong(Buff, ParseIndex, Bytes);
      ...
      if (!strcmp(CmdStr, "InCn"))
      {
        ...
        while (ParseIndex < Bytes)
        {
          ...
          if (Fixt < 9)
          {
            ...
            if (((uint16_t)Val + (uint16_t)Increment) > 255)
              Val = 255;
            else
              Val += Increment;
            DMX_TAR_BUFF[Dmx.ChnlAdd(Fixt, Channel)] = Val;
            Serial.print(Val);
            Serial.print(" ");
            DmxMacro.RunMacro((EEPROM.read(1000) | (EEPROM.read(1001) << 8)));
          }
  ...
  if (BTN_IN_MACROS.Run(BTN_IN.Run()))
  {
    if (BTN_IN_MACROS.State())
    {
      ...
      else if (!ButtonTimerRun)
      {
        ...
        DmxMacro.RunMacro((EEPROM.read(1000) | (EEPROM.read(1001) << 8)));
        ButtonMacroRun++;
        if (ButtonMacroRun >= EEPROM.read(1002))
          ButtonMacroRun = 0;
      }
...
uint32_t ParseLong(char *Data, uint8_t &Index, uint8_t Max)
{
  uint32_t Answer = 0;
  while (Data[Index] >= '0' && Data[Index] <= '9' && Index < Max)
  {
