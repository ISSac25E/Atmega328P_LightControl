...
class VarMacro {
  public:
    ...
    void StopMacro();
    void StepMacro();
    uint8_t StartVals[VarMacro_BYTES];
  private:
...
void VarMacro::RunMacro(uint16_t Steps) {
  if (Steps) {
    _StpsToRun = Steps;
    _CurrStep = 0;
    for (uint16_t X = 0; X < VarMacro_BYTES; X++) StartVals[X] = _CurrPointer[X];
    _RunMacro = true;
  }
  else {
  ...
  if (_RunMacro) {
    if (_StpsToRun && _CurrStep <= _StpsToRun) {
      for (uint16_t X = 0; X < VarMacro_BYTES; X++) {
        uint8_t Offset = ((uint32_t)((uint32_t)_Diff(_TargetPointer[X], StartVals[X]) * (uint32_t)_CurrStep) / _StpsToRun);
        if(StartVals[X] > _TargetPointer[X]) {
          _CurrPointer[X] = StartVals[X] - Offset;
        }
        else {
          _CurrPointer[X] = StartVals[X] + Offset;
        }
